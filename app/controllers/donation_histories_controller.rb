class DonationHistoriesController < ApplicationController
  ...
  def index
    @donation_histories = DonationHistory.all.chronological.paginate(:page => params[:page], :per_page => 30)
    @nav_status = 'db'
  end
  ...
  def import
    DonationHistory.import(params[:file])
    redirect_to donation_histories_path, notice: "csv imported"
  end
